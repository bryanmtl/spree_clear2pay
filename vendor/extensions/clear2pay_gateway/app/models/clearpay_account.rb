class ClearpayAccount < ActiveRecord::Base
  ...
  def capture(payment)
    ...
    if ppx_response.success?
      ClearpayTxn.create(:payment => payment,
                      :txn_type => ClearpayTxn::TxnType::CAPTURE,
  ...
  def credit(payment, amount=nil)
    ...
    if ppx_response.success?
      ...
      ClearpayTxn.new(:paypal_payment => payment,
                    :txn_type => ClearpayTxn::TxnType::CREDIT,
  ...
  def find_authorization(payment)
    ...
    payment.txns.find(:first,
              :conditions => {:pending_reason =>  "authorization", :payment_status => "Pending", :txn_type => ClearpayTxn::TxnType::AUTHORIZE.to_s},
  ...
  def find_capture(payment)
    #find the transaction associated with the original authorization/capture
    payment.txns.find(:first,
              :conditions => {:payment_status => "Completed", :txn_type => ClearpayTxn::TxnType::CAPTURE.to_s},
              :order => 'created_at DESC')
  end
